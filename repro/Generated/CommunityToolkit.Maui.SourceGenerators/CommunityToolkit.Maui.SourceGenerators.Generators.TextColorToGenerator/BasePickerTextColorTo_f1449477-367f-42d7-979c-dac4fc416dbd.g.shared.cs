...
public static partial class ColorAnimationExtensions_BasePicker
{
    /// <summary>
    ...
    public static Task<bool> TextColorTo(this GenericViewBug.BasePicker element, Color color, uint rate = 16u, uint length = 250u, Easing? easing = null)
    {
        ...
        catch (ArgumentException aex)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"{aex.GetType().Name} thrown in {typeof(ColorAnimationExtensions_BasePicker).FullName}: {aex.Message}");
            animationCompletionSource.SetResult(false);
        }

        return animationCompletionSource.Task;
        static Animation GetRedTransformAnimation(GenericViewBug.BasePicker element, float targetRed) => new(v => element.TextColor = element.TextColor.WithRed(v), element.TextColor.Red, targetRed);
        static Animation GetGreenTransformAnimation(GenericViewBug.BasePicker element, float targetGreen) => new(v => element.TextColor = element.TextColor.WithGreen(v), element.TextColor.Green, targetGreen);
        static Animation GetBlueTransformAnimation(GenericViewBug.BasePicker element, float targetBlue) => new(v => element.TextColor = element.TextColor.WithBlue(v), element.TextColor.Blue, targetBlue);
        static Animation GetAlphaTransformAnimation(GenericViewBug.BasePicker element, float targetAlpha) => new(v => element.TextColor = element.TextColor.WithAlpha((float)v), element.TextColor.Alpha, targetAlpha);
    }
}