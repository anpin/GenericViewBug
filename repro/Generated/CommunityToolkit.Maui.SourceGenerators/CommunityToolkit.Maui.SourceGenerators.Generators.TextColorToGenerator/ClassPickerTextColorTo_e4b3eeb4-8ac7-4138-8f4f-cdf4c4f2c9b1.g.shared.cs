...
public static partial class ColorAnimationExtensions_ClassPicker
{
    /// <summary>
    ...
    public static Task<bool> TextColorTo<T>(this GenericViewBug.ClassPicker<T> element, Color color, uint rate = 16u, uint length = 250u, Easing? easing = null)
        where T : class
    {
        ...
        catch (ArgumentException aex)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"{aex.GetType().Name} thrown in {typeof(ColorAnimationExtensions_ClassPicker).FullName}: {aex.Message}");
            animationCompletionSource.SetResult(false);
        }

        return animationCompletionSource.Task;
        static Animation GetRedTransformAnimation(GenericViewBug.ClassPicker<T> element, float targetRed) => new(v => element.TextColor = element.TextColor.WithRed(v), element.TextColor.Red, targetRed);
        static Animation GetGreenTransformAnimation(GenericViewBug.ClassPicker<T> element, float targetGreen) => new(v => element.TextColor = element.TextColor.WithGreen(v), element.TextColor.Green, targetGreen);
        static Animation GetBlueTransformAnimation(GenericViewBug.ClassPicker<T> element, float targetBlue) => new(v => element.TextColor = element.TextColor.WithBlue(v), element.TextColor.Blue, targetBlue);
        static Animation GetAlphaTransformAnimation(GenericViewBug.ClassPicker<T> element, float targetAlpha) => new(v => element.TextColor = element.TextColor.WithAlpha((float)v), element.TextColor.Alpha, targetAlpha);
    }
}