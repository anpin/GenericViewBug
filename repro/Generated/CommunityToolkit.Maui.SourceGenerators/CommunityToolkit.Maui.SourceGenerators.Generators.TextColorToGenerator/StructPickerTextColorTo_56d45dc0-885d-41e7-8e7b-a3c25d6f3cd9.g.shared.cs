...
public static partial class ColorAnimationExtensions_StructPicker
{
    /// <summary>
    ...
    public static Task<bool> TextColorTo<T>(this GenericViewBug.StructPicker<T> element, Color color, uint rate = 16u, uint length = 250u, Easing? easing = null)
        where T : struct
    {
        ...
        catch (ArgumentException aex)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"{aex.GetType().Name} thrown in {typeof(ColorAnimationExtensions_StructPicker).FullName}: {aex.Message}");
            animationCompletionSource.SetResult(false);
        }

        return animationCompletionSource.Task;
        static Animation GetRedTransformAnimation(GenericViewBug.StructPicker<T> element, float targetRed) => new(v => element.TextColor = element.TextColor.WithRed(v), element.TextColor.Red, targetRed);
        static Animation GetGreenTransformAnimation(GenericViewBug.StructPicker<T> element, float targetGreen) => new(v => element.TextColor = element.TextColor.WithGreen(v), element.TextColor.Green, targetGreen);
        static Animation GetBlueTransformAnimation(GenericViewBug.StructPicker<T> element, float targetBlue) => new(v => element.TextColor = element.TextColor.WithBlue(v), element.TextColor.Blue, targetBlue);
        static Animation GetAlphaTransformAnimation(GenericViewBug.StructPicker<T> element, float targetAlpha) => new(v => element.TextColor = element.TextColor.WithAlpha((float)v), element.TextColor.Alpha, targetAlpha);
    }
}